...
type GroupedClients = {
  [username: string]: ClientStatus[];
};

const ClientsTable = ({ clients, user }: ClientsTableProps) => {
  ...
  const deleteClientHandler = async (client: ClientStatus): Promise<void> => {
    setError("");
    if (
      !window.confirm(
  ...
  const downloadClientHandler = async (client: ClientStatus): Promise<void> => {
    setError("");
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to download client config. Please try again.",
      );
    } finally {
      setLoadingId(null);
    }
